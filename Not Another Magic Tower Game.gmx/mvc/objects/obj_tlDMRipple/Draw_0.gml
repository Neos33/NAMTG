draw_set_alpha(1);
if (obj_tlDMRippleController.redrawn) 
{ 
    var width, offset, d, tex, tex_x, tex_y, i, xx, yy;
    width = min(radius,25); 
    offset = (600-radius)/15
    d = 360/80;

    tex = surface_get_texture(obj_tlDMRippleController.buffer_surface);
    tex_x = texture_get_width(tex)/surface_get_width(obj_tlDMRippleController.buffer_surface);
    tex_y = texture_get_height(tex)/surface_get_height(obj_tlDMRippleController.buffer_surface);
