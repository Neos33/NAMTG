...
if(speaker != "") {
    ...
    draw_roundrect(__view_get( e__VW.XView, 0 ),
        dHeight - 32,
        __view_get( e__VW.XView, 0 ) + string_width(string_hash_to_newline(speaker)) + 16,
        dHeight,
        false);
    ...
    draw_text(__view_get( e__VW.XView, 0 ) + 8, dHeight - 28, string_hash_to_newline(speaker));
}
    ...
    if(char == "%") {
        ...
        if(colChange) {
            ...
            if(char != "#") {
                ...
                draw_text(__view_get( e__VW.XView, 0 ) + 24 + (charX mod 720), dHeight + 16 + charY + floor(charX / 720) * 24, string_hash_to_newline(char));
                charX += string_width(string_hash_to_newline(char));
            } else {
                //Start the next line
                charY += string_height(string_hash_to_newline("A")); //'A' just used to get the letter height
